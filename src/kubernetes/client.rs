...
impl KubeClient {
    ...
    pub async fn get_pod_logs(
        ...
        let logs = api
            .logs(name, &params)
            .await
            .context(format!("Failed to get logs for pod {} in namespace {}", name, namespace))?;
    ...
    pub async fn force_delete_pod(&self, name: &str, namespace: &str) -> Result<()> {
        ...
        api.delete(name, &dp)
            .await
            .context(format!("Failed to force delete pod {} in namespace {}", name, namespace))?;
        Ok(())
    }
                ...
                for cs in container_statuses {
                    ...
                    if let Some(state) = &cs.state {
                        if let Some(running) = &state.running {
                            ...
                            output.push_str(&format!("    State:          Waiting\n"));
                            output.push_str(&format!("      Reason:       {}\n", waiting.reason.as_deref().unwrap_or("")));
                        } else if let Some(terminated) = &state.terminated {
                            output.push_str(&format!("    State:          Terminated\n"));
    ...
    pub async fn get_service_yaml(&self, name: &str, namespace: &str) -> Result<String> {
        ...
        let svc = api
            .get(name)
            .await
            .context(format!("Failed to get service {}", name))?;
    ...
    pub async fn delete_deployment(&self, name: &str, namespace: &str) -> Result<()> {
        ...
        api.delete(name, &DeleteParams::default())
            .await
            .context(format!("Failed to delete deployment {} in namespace {}", name, namespace))?;
        Ok(())
    }
    ...
    pub async fn create_deployment(&self, options: CreateDeploymentOptions) -> Result<String> {
        ...
        use k8s_openapi::api::core::v1::{Container, ContainerPort, EnvVar, PodSpec, PodTemplateSpec, ResourceRequirements};
        ...
        let container = Container {
            ...
            args: if options.args.is_empty() { None } else { Some(options.args.clone()) },
            ..Default::default()
        };
        ...
        Ok(format!("Deployment {} created in namespace {}", options.name, namespace))
    }
