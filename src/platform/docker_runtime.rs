...
impl DockerRuntime {
    ...
    pub fn is_available(&self) -> bool {
        match self {
            Self::Colima { profile } => {
                let home = dirs::home_dir().unwrap_or_default();
                let socket = format!("{}/.colima/{}/docker.sock", home.display(), profile);
                Path::new(&socket).exists()
            }
            Self::NativeDocker { socket_path } | Self::RootlessDocker { socket_path } => Path::new(socket_path).exists(),
            Self::Wsl2Docker { .. } => {
                // For TCP connections, we can't easily check without attempting a connection
                // Return true and let the actual connection attempt determine availability
                true
            }
            Self::Custom { connection_string } => {
                if connection_string.starts_with("http://") || connection_string.starts_with("tcp://") {
                    true // Can't easily check TCP without connecting
                } else {
                    Path::new(connection_string).exists()
                }
            }
        }
    }
...
mod tests {
    ...
    fn test_detect_available_returns_list() {
        let runtimes = DockerRuntime::detect_available();
