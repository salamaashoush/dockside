...
pub fn open_favorite(item: &FavoriteRef, cx: &mut App) {
  ...
  match item {
    ...
    FavoriteRef::CronJob { name, namespace } => {
      state.update(cx, |s, cx| {
        s.set_view(CurrentView::CronJobs);
        s.set_selection(Selection::CronJob {
          name: name.clone(),
          namespace: namespace.clone(),
        });
        cx.emit(StateChanged::ViewChanged);
    ...
    FavoriteRef::Ingress { name, namespace } => {
      state.update(cx, |s, cx| {
        s.set_view(CurrentView::Ingresses);
        s.set_selection(Selection::Ingress {
          name: name.clone(),
          namespace: namespace.clone(),
        });
        cx.emit(StateChanged::ViewChanged);
    ...
    FavoriteRef::Pvc { name, namespace } => {
      state.update(cx, |s, cx| {
        s.set_view(CurrentView::Pvcs);
        s.set_selection(Selection::Pvc {
          name: name.clone(),
          namespace: namespace.clone(),
        });
        cx.emit(StateChanged::ViewChanged);
    ...
    FavoriteRef::Secret { name, namespace } => {
      state.update(cx, |s, cx| {
        s.set_view(CurrentView::Secrets);
        s.set_selection(Selection::Secret {
          name: name.clone(),
          namespace: namespace.clone(),
        });
        cx.emit(StateChanged::ViewChanged);
    ...
    FavoriteRef::ConfigMap { name, namespace } => {
      state.update(cx, |s, cx| {
        s.set_view(CurrentView::ConfigMaps);
        s.set_selection(Selection::ConfigMap {
          name: name.clone(),
          namespace: namespace.clone(),
        });
        cx.emit(StateChanged::ViewChanged);
