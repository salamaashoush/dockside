...
pub fn get_pvc_yaml(name: String, namespace: String, cx: &mut App) {
  let state = docker_state(cx);
  ...
  let tokio_task = Tokio::spawn(cx, async move {
    let client = crate::kubernetes::KubeClient::new().await?;
    client.get_pvc_yaml(&name, &namespace).await
  });
  ...
  cx.spawn(async move |cx| {
    ...
    cx.update(|cx| {
      state.update(cx, |_state, cx| {
        cx.emit(StateChanged::PvcYamlLoaded {
          name: name_clone,
...
pub fn apply_pvc_yaml(name: String, namespace: String, yaml: String, cx: &mut App) {
  let task_id = start_task(cx, format!("Applying PVC '{name}'..."));
  let disp = dispatcher(cx);
  let label = name.clone();
  let tokio_task = Tokio::spawn(cx, async move {
    let client = crate::kubernetes::KubeClient::new().await?;
    client.apply_pvc_yaml(&name, &namespace, &yaml).await
  });
  cx.spawn(async move |cx| {
    let result = tokio_task.await.unwrap_or_else(|e| Err(anyhow::anyhow!("{e}")));
      ...
      Ok(()) => {
        ...
        disp.update(cx, |_, cx| {
          cx.emit(DispatcherEvent::TaskCompleted {
            message: format!("PVC '{label}' applied"),
          });
        });
        refresh_pvcs(cx);
      }
      Err(e) => {
        ...
        disp.update(cx, |_, cx| {
          cx.emit(DispatcherEvent::TaskFailed {
            error: format!("Failed to apply PVC '{label}': {e}"),
          });
        });
      }
    })
