...
pub fn get_secret_yaml(name: String, namespace: String, cx: &mut App) {
  let state = docker_state(cx);
  ...
  let tokio_task = Tokio::spawn(cx, async move {
    let client = crate::kubernetes::KubeClient::new().await?;
    client.get_secret_yaml(&name, &namespace).await
  });
  ...
  cx.spawn(async move |cx| {
    ...
    cx.update(|cx| {
      state.update(cx, |_state, cx| {
        cx.emit(StateChanged::SecretYamlLoaded {
          name: name_clone,
...
pub fn apply_secret_yaml(name: String, namespace: String, yaml: String, cx: &mut App) {
  let task_id = start_task(cx, format!("Applying secret '{name}'..."));
  let disp = dispatcher(cx);
  let label = name.clone();
  let tokio_task = Tokio::spawn(cx, async move {
    let client = crate::kubernetes::KubeClient::new().await?;
    client.apply_secret_yaml(&name, &namespace, &yaml).await
  });
  cx.spawn(async move |cx| {
    let result = tokio_task.await.unwrap_or_else(|e| Err(anyhow::anyhow!("{e}")));
      ...
      Ok(()) => {
        ...
        disp.update(cx, |_, cx| {
          cx.emit(DispatcherEvent::TaskCompleted {
            message: format!("Secret '{label}' applied"),
          });
        });
        refresh_secrets(cx);
      }
      Err(e) => {
        ...
        disp.update(cx, |_, cx| {
          cx.emit(DispatcherEvent::TaskFailed {
            error: format!("Failed to apply secret '{label}': {e}"),
          });
        });
      }
    })
...
pub fn create_secret(name: String, namespace: String, entries: Vec<(String, String)>, cx: &mut App) {
  let task_id = start_task(cx, format!("Creating secret '{name}'..."));
  let disp = dispatcher(cx);
  let label = name.clone();
  let tokio_task = Tokio::spawn(cx, async move {
    let client = crate::kubernetes::KubeClient::new().await?;
    client.create_secret(&name, &namespace, &entries).await
  });
  cx.spawn(async move |cx| {
    let result = tokio_task.await.unwrap_or_else(|e| Err(anyhow::anyhow!("{e}")));
    cx.update(|cx| match result {
      Ok(()) => {
        ...
        disp.update(cx, |_, cx| {
          cx.emit(DispatcherEvent::TaskCompleted {
            message: format!("Secret '{label}' created"),
          });
        });
        refresh_secrets(cx);
      }
      Err(e) => {
        ...
        disp.update(cx, |_, cx| {
          cx.emit(DispatcherEvent::TaskFailed {
            error: format!("Failed to create secret '{label}': {e}"),
          });
        });
      }
    })
