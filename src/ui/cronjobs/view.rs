...
use super::detail::CronJobDetail;
use super::list::{CronJobList, CronJobListEvent};
...
pub struct CronJobsView {
  ...
  list: Entity<CronJobList>,
  detail: Entity<CronJobDetail>,
}

impl CronJobsView {
  fn selected(&self, cx: &App) -> Option<CronJobInfo> {
    let state = self.docker_state.read(cx);
    if let Selection::CronJob {
      ref name,
      ref namespace,
    } = state.selection
    {
      state.get_cronjob(name, namespace).cloned()
    } else {
      None
    }
  }
  ...
  pub fn new(window: &mut Window, cx: &mut Context<'_, Self>) -> Self {
    ...
    let list = cx.new(|cx| CronJobList::new(window, cx));
    let detail = cx.new(CronJobDetail::new);
      ...
      |this, _list, event: &CronJobListEvent, _window, cx| match event {
        CronJobListEvent::Selected(item) => {
          ...
          this.docker_state.update(cx, |state, _cx| {
            state.set_selection(Selection::CronJob {
    ...
    cx.subscribe_in(
      ...
      |this, ds, event: &StateChanged, _w, cx| match event {
        StateChanged::CronJobTabRequest {
          name,
          namespace,
          tab: _,
        } => {
          let opt = ds.read(cx).get_cronjob(name, namespace).cloned();
        ...
        StateChanged::CronJobsUpdated => {
          let key = if let Selection::CronJob {
            ref name,
            ref namespace,
          } = this.docker_state.read(cx).selection
          {
