...
pub enum IngressListEvent {
  Selected(IngressInfo),
}

pub struct IngressListDelegate {
  docker_state: Entity<DockerState>,
  search_query: String,
}

impl IngressListDelegate {
  fn items(&self, cx: &App) -> Vec<IngressInfo> {
    let state = self.docker_state.read(cx);
    if state.selected_namespace == "all" {
      state.ingresses.clone()
    } else {
      state
        .ingresses
        .iter()
        .filter(|i| i.namespace == state.selected_namespace)
        .cloned()
        .collect()
    }
  }

  fn filtered(&self, cx: &App) -> Vec<IngressInfo> {
    ...
    items
      .into_iter()
      .filter(|i| i.name.to_lowercase().contains(&q) || i.namespace.to_lowercase().contains(&q))
      .collect()
  }
...
impl ListDelegate for IngressListDelegate {
  type Item = ListItem;
  ...
  fn render_item(
    ...
    let i = items.get(ix.row)?;
    let colors = &cx.theme().colors;

    let global_selection = &self.docker_state.read(cx).selection;
    let is_selected = matches!(global_selection, Selection::Ingress { name, namespace } if *name == i.name && *namespace == i.namespace);
    let name_clone = i.name.clone();
    let ns_clone = i.namespace.clone();
    let pin_favorite = crate::state::FavoriteRef::Ingress {
      name: i.name.clone(),
    ...
    let subtitle = if i.hosts.is_empty() {
      i.namespace.clone()
    } else {
      format!("{} • {}", i.namespace, i.hosts.join(", "))
    };
      ...
      .dropdown_menu(move |menu, _window, _cx| {
        ...
        menu
          .item(PopupMenuItem::new("View YAML").icon(IconName::File).on_click({
            ...
            let ns = ns.clone();
            move |_, _, cx| {
              services::open_ingress_yaml(name.clone(), ns.clone(), cx);
            }
          }))
          .separator()
          .item(
          ...
          .item(PopupMenuItem::new("Delete").icon(Icon::new(AppIcon::Trash)).on_click({
            ...
            let ns = ns.clone();
            move |_, _, cx| {
              services::delete_ingress(name.clone(), ns.clone(), cx);
            }
          }))
      });
...
pub struct IngressList {
  docker_state: Entity<DockerState>,
  list_state: Entity<ListState<IngressListDelegate>>,
...
impl IngressList {
  pub fn new(window: &mut Window, cx: &mut Context<'_, Self>) -> Self {
    ...
    let delegate = IngressListDelegate {
    ...
    cx.subscribe(&list_state, |_this, state, event: &ListEvent, cx| match event {
      ListEvent::Select(ix) | ListEvent::Confirm(ix) => {
        ...
        if let Some(i) = filtered.get(ix.row) {
          cx.emit(IngressListEvent::Selected(i.clone()));
        }
      }
      ListEvent::Cancel => {}
    })
  ...
  fn ensure_search_input(&mut self, window: &mut Window, cx: &mut Context<'_, Self>) {
    if self.search_input.is_none() {
      let input_state = cx.new(|cx| InputState::new(window, cx).placeholder("Search ingresses..."));
...
impl gpui::EventEmitter<IngressListEvent> for IngressList {}

impl Render for IngressList {
  fn render(&mut self, window: &mut Window, cx: &mut Context<'_, Self>) -> impl IntoElement {
    let state = self.docker_state.read(cx);
    let total_count = state.ingresses.len();
    ...
    let toolbar = h_flex()
      ...
      .child(
        h_flex()
          ...
          .child(
            Button::new("ing-toolbar-actions")
              .icon(IconName::Ellipsis)
              .ghost()
              .compact()
              .dropdown_menu(|menu, _, _| {
                menu.item(
                  PopupMenuItem::new("Refresh")
                    ...
                    .on_click(|_, _, cx| services::refresh_ingresses(cx)),
                )
              }),
          ),
      );
