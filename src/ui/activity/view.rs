...
impl ActivityMonitorView {
    ...
    fn render_container_group(&self, cx: &Context<Self>) -> impl IntoElement {
        ...
        let total_network: u64 = self.stats.container_stats.iter().map(|s| s.network_rx + s.network_tx).sum();
        let total_disk: u64 = self.stats.container_stats.iter().map(|s| s.block_read + s.block_write).sum();
        ...
        v_flex()
            ...
            .child(
                ...
                div()
                    .flex_1()
                    .mt(px(8.))
                    .child(self.render_mini_chart(history, color)),
            )
    }
