...
use crate::kubernetes::StatefulSetInfo;
use crate::services;
...
pub enum StatefulSetListEvent {
  Selected(StatefulSetInfo),
}

pub struct StatefulSetListDelegate {
  docker_state: Entity<DockerState>,
  search_query: String,
}

impl StatefulSetListDelegate {
  fn items(&self, cx: &App) -> Vec<StatefulSetInfo> {
    let state = self.docker_state.read(cx);
    if state.selected_namespace == "all" {
      state.statefulsets.clone()
    } else {
      state
        .statefulsets
        .iter()
  ...
  fn filtered(&self, cx: &App) -> Vec<StatefulSetInfo> {
    ...
    items
      .into_iter()
      .filter(|s| s.name.to_lowercase().contains(&q) || s.namespace.to_lowercase().contains(&q))
      .collect()
  }
...
impl ListDelegate for StatefulSetListDelegate {
  type Item = ListItem;
  ...
  fn render_item(
    ...
    let s = items.get(ix.row)?;
    let colors = &cx.theme().colors;

    let global_selection = &self.docker_state.read(cx).selection;
    let is_selected = matches!(global_selection, Selection::StatefulSet { name, namespace } if *name == s.name && *namespace == s.namespace);
    let name_clone = s.name.clone();
    let ns_clone = s.namespace.clone();
    let pin_favorite = crate::state::FavoriteRef::StatefulSet {
      name: s.name.clone(),
      namespace: s.namespace.clone(),
    };
    let pinned = services::is_favorite(&pin_favorite, cx);

    let is_system = matches!(s.namespace.as_str(), "kube-system" | "kube-public" | "kube-node-lease");

    let is_healthy = s.ready_replicas == s.replicas && s.replicas > 0;
    let icon_bg = if is_healthy {
      colors.success
    } else if s.ready_replicas > 0 {
      colors.warning
    } else {
      colors.danger
    };
    let ready_display = s.ready_display();
    let subtitle = format!("{} - {}", s.namespace, s.age);
    let current_replicas = s.replicas;
    ...
    let menu_button = Button::new(("menu", row))
      ...
      .dropdown_menu(move |menu, _window, _cx| {
        ...
        let mut menu = menu
          .item(
            PopupMenuItem::new(format!("Scale to {}", current_replicas + 1)).on_click({
              let name = name.clone();
              let ns = ns.clone();
              move |_, _, cx| {
                services::scale_statefulset(name.clone(), ns.clone(), current_replicas + 1, cx);
              }
            }),
          )
          .item(
            PopupMenuItem::new(format!("Scale to {}", (current_replicas - 1).max(0))).on_click({
              let name = name.clone();
              let ns = ns.clone();
              move |_, _, cx| {
                services::scale_statefulset(name.clone(), ns.clone(), (current_replicas - 1).max(0), cx);
              }
            }),
          )
          .item(
            PopupMenuItem::new("Rolling Restart")
              ...
              .on_click({
                ...
                let ns = ns.clone();
                move |_, _, cx| {
                  services::rollout_restart_statefulset(name.clone(), ns.clone(), cx);
                }
              }),
          )
          .separator()
          .item(PopupMenuItem::new("View YAML").icon(IconName::File).on_click({
            ...
            let ns = ns.clone();
            move |_, _, cx| {
              services::open_statefulset_yaml(name.clone(), ns.clone(), cx);
            }
          }))
          .separator()
          .item(
        ...
        if !is_system {
          menu = menu
            ...
            .item(PopupMenuItem::new("Delete").icon(Icon::new(AppIcon::Trash)).on_click({
              ...
              let ns = ns.clone();
              move |_, _, cx| {
                services::delete_statefulset(name.clone(), ns.clone(), cx);
              }
            }));
        }
...
pub struct StatefulSetList {
  docker_state: Entity<DockerState>,
  list_state: Entity<ListState<StatefulSetListDelegate>>,
...
impl StatefulSetList {
  pub fn new(window: &mut Window, cx: &mut Context<'_, Self>) -> Self {
    ...
    let delegate = StatefulSetListDelegate {
    ...
    cx.subscribe(&list_state, |_this, state, event: &ListEvent, cx| match event {
      ListEvent::Select(ix) | ListEvent::Confirm(ix) => {
        ...
        if let Some(s) = filtered.get(ix.row) {
          cx.emit(StatefulSetListEvent::Selected(s.clone()));
        }
      }
      ListEvent::Cancel => {}
    })
  ...
  fn ensure_search_input(&mut self, window: &mut Window, cx: &mut Context<'_, Self>) {
    if self.search_input.is_none() {
      let input_state = cx.new(|cx| InputState::new(window, cx).placeholder("Search statefulsets..."));
    ...
    v_flex()
      ...
      .child(
        div()
          ...
          .child("No StatefulSets"),
      )
  }
}

impl gpui::EventEmitter<StatefulSetListEvent> for StatefulSetList {}

impl Render for StatefulSetList {
  fn render(&mut self, window: &mut Window, cx: &mut Context<'_, Self>) -> impl IntoElement {
    let state = self.docker_state.read(cx);
    let total_count = state.statefulsets.len();
    let load_state = state.statefulsets_state.clone();
              ...
              .dropdown_menu(|menu, _, _| {
                menu.item(
                  PopupMenuItem::new("Refresh")
                    ...
                    .on_click(|_, _, cx| services::refresh_statefulsets(cx)),
                )
              }),
          ),
      );
    ...
    let content: gpui::Div = match &load_state {
      LoadState::NotLoaded | LoadState::Loading => render_loading("statefulsets", cx),
      LoadState::Error(e) => render_k8s_error(
        "statefulsets",
        &e.clone(),
        |_ev, _w, cx| services::refresh_statefulsets(cx),
        cx,
      ),
      LoadState::Loaded => {
        if empty {
