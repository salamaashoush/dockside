...
impl StatefulSetsView {
  pub fn new(_window: &mut Window, cx: &mut Context<'_, Self>) -> Self {
    ...
    cx.subscribe(&docker_state, |_this, _state, event: &StateChanged, cx| {
      if matches!(
        event,
        StateChanged::StatefulSetsUpdated | StateChanged::NamespacesUpdated
      ) {
        cx.notify();
      }
    })
    ...
    cx.spawn(async move |_this, cx| {
      loop {
        ...
        let _ = cx.update(services::refresh_statefulsets);
      }
    })
    .detach();

    services::refresh_statefulsets(cx);
...
impl Render for StatefulSetsView {
  fn render(&mut self, _window: &mut Window, cx: &mut Context<'_, Self>) -> impl IntoElement {
    let colors = cx.theme().colors;
    let state = self.docker_state.read(cx);
    let items = state.statefulsets.clone();
    ...
    let toolbar = h_flex()
      ...
      .justify_between()
      .child(
        v_flex().child(Label::new("StatefulSets")).child(
          div()
            .text_xs()
        ...
        h_flex()
          ...
          .child(
            Button::new("refresh")
              ...
              .ghost()
              .compact()
              .on_click(cx.listener(|_this, _ev, _w, cx| services::refresh_statefulsets(cx))),
          ),
      );
        ...
        if items.is_empty() {
          div().size_full().flex().items_center().justify_center().child(
            div()
              .text_sm()
              .text_color(colors.muted_foreground)
              .child("No statefulsets in selected namespace."),
          )
        } else {
              ...
              h_flex()
                ...
                .child(
                  div()
                    ...
                    .text_ellipsis()
                    .overflow_hidden()
                    .child(s.images.first().cloned().unwrap_or_default()),
                )
                .child(
                  Button::new(SharedString::from(format!("menu-{name}-{namespace}")))
                    ...
                    .dropdown_menu({
                      ...
                      move |menu, _, _| {
                        ...
                        let scale_up_name = n.clone();
                        let scale_up_namespace = ns.clone();
                        let scale_down_name = n.clone();
                        let scale_down_namespace = ns.clone();
                        ...
                        menu
                          .item(
                            PopupMenuItem::new("Rolling Restart")
                              ...
                              .on_click(move |_, _, cx| {
                                services::rollout_restart_statefulset(
                                  restart_name.clone(),
                          ...
                          .item(
                            PopupMenuItem::new(format!("Scale to {}", replicas + 1))
                              .icon(Icon::new(AppIcon::Plus))
                              .on_click(move |_, _, cx| {
                                services::scale_statefulset(
                                  scale_up_name.clone(),
                                  scale_up_namespace.clone(),
                                  replicas + 1,
                                  cx,
                                );
                              }),
                          ...
                          .item(
                            PopupMenuItem::new(format!("Scale to {}", (replicas - 1).max(0))).on_click(
                              move |_, _, cx| {
                                services::scale_statefulset(
                                  scale_down_name.clone(),
                                  scale_down_namespace.clone(),
                                  (replicas - 1).max(0),
                                  cx,
                                );
                              },
                          ...
                          .item(PopupMenuItem::new("Delete").icon(Icon::new(AppIcon::Trash)).on_click(
                            move |_, _, cx| {
                              services::delete_statefulset(delete_name.clone(), delete_namespace.clone(), cx);
                            },
                          ))
                      }
                    }),
