...
pub enum DaemonSetListEvent {
  Selected(DaemonSetInfo),
}

pub struct DaemonSetListDelegate {
  docker_state: Entity<DockerState>,
  search_query: String,
}

impl DaemonSetListDelegate {
  fn items(&self, cx: &App) -> Vec<DaemonSetInfo> {
    let state = self.docker_state.read(cx);
    if state.selected_namespace == "all" {
      state.daemonsets.clone()
    } else {
      state
        .daemonsets
        .iter()
  ...
  fn filtered(&self, cx: &App) -> Vec<DaemonSetInfo> {
    ...
    items
      .into_iter()
      .filter(|d| d.name.to_lowercase().contains(&q) || d.namespace.to_lowercase().contains(&q))
      .collect()
  }
...
impl ListDelegate for DaemonSetListDelegate {
  type Item = ListItem;
  ...
  fn render_item(
    ...
    let d = items.get(ix.row)?;
    let colors = &cx.theme().colors;

    let global_selection = &self.docker_state.read(cx).selection;
    let is_selected = matches!(global_selection, Selection::DaemonSet { name, namespace } if *name == d.name && *namespace == d.namespace);
    let name_clone = d.name.clone();
    let ns_clone = d.namespace.clone();
    let pin_favorite = crate::state::FavoriteRef::DaemonSet {
      name: d.name.clone(),
      namespace: d.namespace.clone(),
    };
    let pinned = services::is_favorite(&pin_favorite, cx);

    let is_system = matches!(d.namespace.as_str(), "kube-system" | "kube-public" | "kube-node-lease");

    let is_healthy = d.ready == d.desired && d.desired > 0;
    ...
    let ready_display = d.ready_display();
    let subtitle = format!("{} - {}", d.namespace, d.age);
      ...
      .dropdown_menu(move |menu, _window, _cx| {
        ...
        let mut menu = menu
          .item(
            PopupMenuItem::new("Rolling Restart")
              ...
              .on_click({
                ...
                let ns = ns.clone();
                move |_, _, cx| {
                  services::rollout_restart_daemonset(name.clone(), ns.clone(), cx);
                }
              }),
          )
          .separator()
          .item(PopupMenuItem::new("View YAML").icon(IconName::File).on_click({
            ...
            let ns = ns.clone();
            move |_, _, cx| {
              services::open_daemonset_yaml(name.clone(), ns.clone(), cx);
            }
          }))
          .separator()
          .item(
        ...
        if !is_system {
          menu = menu
            ...
            .item(PopupMenuItem::new("Delete").icon(Icon::new(AppIcon::Trash)).on_click({
              ...
              let ns = ns.clone();
              move |_, _, cx| {
                services::delete_daemonset(name.clone(), ns.clone(), cx);
              }
            }));
        }
...
pub struct DaemonSetList {
  docker_state: Entity<DockerState>,
  list_state: Entity<ListState<DaemonSetListDelegate>>,
...
impl DaemonSetList {
  pub fn new(window: &mut Window, cx: &mut Context<'_, Self>) -> Self {
    ...
    let delegate = DaemonSetListDelegate {
    ...
    cx.subscribe(&list_state, |_this, state, event: &ListEvent, cx| match event {
      ListEvent::Select(ix) | ListEvent::Confirm(ix) => {
        ...
        if let Some(d) = filtered.get(ix.row) {
          cx.emit(DaemonSetListEvent::Selected(d.clone()));
        }
      }
      ListEvent::Cancel => {}
    })
  ...
  fn ensure_search_input(&mut self, window: &mut Window, cx: &mut Context<'_, Self>) {
    if self.search_input.is_none() {
      let input_state = cx.new(|cx| InputState::new(window, cx).placeholder("Search daemonsets..."));
...
impl gpui::EventEmitter<DaemonSetListEvent> for DaemonSetList {}

impl Render for DaemonSetList {
  fn render(&mut self, window: &mut Window, cx: &mut Context<'_, Self>) -> impl IntoElement {
    let state = self.docker_state.read(cx);
    let total_count = state.daemonsets.len();
              ...
              .dropdown_menu(|menu, _, _| {
                menu.item(
                  PopupMenuItem::new("Refresh")
                    ...
                    .on_click(|_, _, cx| services::refresh_daemonsets(cx)),
                )
              }),
          ),
      );
