...
pub enum PvcListEvent {
  Selected(PvcInfo),
}

pub struct PvcListDelegate {
  docker_state: Entity<DockerState>,
  search_query: String,
}

impl PvcListDelegate {
  fn items(&self, cx: &App) -> Vec<PvcInfo> {
  ...
  fn filtered(&self, cx: &App) -> Vec<PvcInfo> {
    ...
    items
      .into_iter()
      .filter(|p| p.name.to_lowercase().contains(&q) || p.namespace.to_lowercase().contains(&q))
      .collect()
  }
...
impl ListDelegate for PvcListDelegate {
  type Item = ListItem;
  ...
  fn render_item(
    ...
    let p = items.get(ix.row)?;
    let colors = &cx.theme().colors;

    let global_selection = &self.docker_state.read(cx).selection;
    let is_selected =
      matches!(global_selection, Selection::Pvc { name, namespace } if *name == p.name && *namespace == p.namespace);
    let name_clone = p.name.clone();
    let ns_clone = p.namespace.clone();
    let pin_favorite = crate::state::FavoriteRef::Pvc {
      name: p.name.clone(),
    ...
    let capacity = if p.capacity.is_empty() {
      "—".to_string()
    } else {
      p.capacity.clone()
    };
    let subtitle = format!("{} • {} • {}", p.namespace, p.status, capacity);
      ...
      .dropdown_menu(move |menu, _window, _cx| {
        ...
        menu
          .item(PopupMenuItem::new("View YAML").icon(IconName::File).on_click({
            ...
            let ns = ns.clone();
            move |_, _, cx| {
              services::open_pvc_yaml(name.clone(), ns.clone(), cx);
            }
          }))
          .separator()
          .item(
          ...
          .item(PopupMenuItem::new("Delete").icon(Icon::new(AppIcon::Trash)).on_click({
            ...
            let ns = ns.clone();
            move |_, _, cx| {
              services::delete_pvc(name.clone(), ns.clone(), cx);
            }
          }))
      });
...
pub struct PvcList {
  docker_state: Entity<DockerState>,
  list_state: Entity<ListState<PvcListDelegate>>,
...
impl PvcList {
  pub fn new(window: &mut Window, cx: &mut Context<'_, Self>) -> Self {
    ...
    cx.subscribe(&list_state, |_this, state, event: &ListEvent, cx| match event {
      ListEvent::Select(ix) | ListEvent::Confirm(ix) => {
        ...
        if let Some(p) = filtered.get(ix.row) {
          cx.emit(PvcListEvent::Selected(p.clone()));
        }
      }
      ListEvent::Cancel => {}
    })
  ...
  fn ensure_search_input(&mut self, window: &mut Window, cx: &mut Context<'_, Self>) {
    if self.search_input.is_none() {
      let input_state = cx.new(|cx| InputState::new(window, cx).placeholder("Search PVCs..."));
...
impl gpui::EventEmitter<PvcListEvent> for PvcList {}

impl Render for PvcList {
  fn render(&mut self, window: &mut Window, cx: &mut Context<'_, Self>) -> impl IntoElement {
    let state = self.docker_state.read(cx);
    let total_count = state.pvcs.len();
    ...
    let toolbar = h_flex()
      ...
      .child(
        h_flex()
          ...
          .child(
            Button::new("pvc-toolbar-actions")
              .icon(IconName::Ellipsis)
              .ghost()
              .compact()
              .dropdown_menu(|menu, _, _| {
                menu.item(
                  PopupMenuItem::new("Refresh")
                    ...
                    .on_click(|_, _, cx| services::refresh_pvcs(cx)),
                )
              }),
          ),
      );
