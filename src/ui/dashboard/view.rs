...
impl DashboardView {
  ...
  fn render_dns_card(&self, cx: &mut Context<'_, Self>) -> gpui::Div {
    let colors = cx.theme().colors;
    let settings = self.settings_state.read(cx).settings.clone();
    let dns_mgr = services::dns::manager(cx);
    let dns_running = dns_mgr.is_running();
    let proxy_mgr = services::proxy::manager(cx);
    let proxy_running = proxy_mgr.is_running();
    let routes: Vec<(String, String)> = dns_mgr
      .route_map()
      .map_or_else(Vec::new, |m| m.read().route_summaries());
        ...
        h_flex()
          ...
          .child(
            div()
              .text_xs()
              .text_color(colors.muted_foreground)
              .child(format!("*.{}", settings.dns_suffix.trim_matches('.'))),
          ),
      )
      .child(
        Button::new("dns-card-settings")
          .label("Configure")
          .ghost()
          .compact()
          .on_click(cx.listener(|_this, _ev, _w, cx| {
            services::set_view(CurrentView::Settings, cx);
          })),
      );

    let suffix = settings.dns_suffix.trim_matches('.').to_string();
    let port = settings.proxy_http_port;

    let mut list = v_flex().w_full().gap(px(4.));
    if !settings.dns_enabled {
      list = list.child(
        div()
          .text_xs()
          .text_color(colors.muted_foreground)
          .child("Toggle on in Settings → Local DNS to start routing containers."),
      );
      ...
      list = list.child(
        div()
          .text_xs()
          .text_color(colors.muted_foreground)
          .child("No running containers yet. Start one with `docker run -d --name nginx -p 8080:80 nginx`."),
      );
    } else {
      for (name, target) in routes.iter().take(8) {
        let url = if port == 80 {
          format!("http://{name}.{suffix}/")
        } else {
          format!("http://{name}.{suffix}:{port}/")
        };
        let url_for_click = url.clone();
        let id_text = format!("dns-row-{name}");
        list = list.child(
          h_flex()
            .w_full()
            .py(px(4.))
      ...
      if routes.len() > 8 {
        list = list.child(div().text_xs().text_color(colors.muted_foreground).child(format!(
          "+ {} more — see Settings → Local DNS → Routes",
          routes.len() - 8
        )));
      }
    }
