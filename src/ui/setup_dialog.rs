...
pub fn is_docker_running() -> bool {
  ...
  if platform.uses_unix_socket() {
    // Native or rootless Docker socket
    if let Some(socket) = crate::platform::get_default_docker_socket()
  ...
  // TCP-only platforms (Windows) and Unix-socket fallback both probe via `docker info`
  crate::utils::docker_cmd()
    .arg("info")
    .output()
    .is_ok_and(|o| o.status.success())
...
impl Render for SetupDialog {
  fn render(&mut self, _window: &mut Window, cx: &mut Context<'_, Self>) -> impl IntoElement {
    ...
    // Show what platform we detected and how Docker connects on it.
    let connection_note = if platform.requires_tcp_connection() {
      "Docker is reached over TCP from native Windows."
    } else {
      "Docker is reached over its Unix socket."
    };
            ...
            .child(
                v_flex()
                    ...
                    .child(
                        h_flex()
                            .gap(px(6.))
                            .child(div().text_xs().text_color(colors.muted_foreground).child("Platform:"))
                            .child(div().text_xs().child(platform.display_name())),
                    )
                    .child(div().text_xs().text_color(colors.muted_foreground).child(connection_note)),
            )
