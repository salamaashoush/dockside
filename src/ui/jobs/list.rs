...
pub enum JobListEvent {
  Selected(JobInfo),
}

pub struct JobListDelegate {
  docker_state: Entity<DockerState>,
  search_query: String,
}

impl JobListDelegate {
  fn items(&self, cx: &App) -> Vec<JobInfo> {
    ...
    if state.selected_namespace == "all" {
      ...
      state
        .jobs
        .iter()
        .filter(|j| j.namespace == state.selected_namespace)
        .cloned()
        .collect()
    }
  }

  fn filtered(&self, cx: &App) -> Vec<JobInfo> {
    ...
    items
      .into_iter()
      .filter(|j| j.name.to_lowercase().contains(&q) || j.namespace.to_lowercase().contains(&q))
      .collect()
  }
...
impl ListDelegate for JobListDelegate {
  ...
  fn render_item(
    ...
    let j = items.get(ix.row)?;
    let colors = &cx.theme().colors;

    let global_selection = &self.docker_state.read(cx).selection;
    let is_selected =
      matches!(global_selection, Selection::Job { name, namespace } if *name == j.name && *namespace == j.namespace);
    let name_clone = j.name.clone();
    let ns_clone = j.namespace.clone();
    let pin_favorite = crate::state::FavoriteRef::Job {
      name: j.name.clone(),
      namespace: j.namespace.clone(),
    };
    let pinned = services::is_favorite(&pin_favorite, cx);

    let icon_bg = if j.failed > 0 {
      colors.danger
    } else if j.succeeded >= j.completions {
      colors.success
    } else {
      colors.warning
    };
    let completions_display = j.completions_display();
    let subtitle = format!("{} - {}", j.namespace, j.duration);
      ...
      .dropdown_menu(move |menu, _window, _cx| {
        ...
        menu
          .item(PopupMenuItem::new("View YAML").icon(IconName::File).on_click({
            ...
            let ns = ns.clone();
            move |_, _, cx| {
              services::open_job_yaml(name.clone(), ns.clone(), cx);
            }
          }))
          .separator()
          .item(
          ...
          .item(PopupMenuItem::new("Delete").icon(Icon::new(AppIcon::Trash)).on_click({
            ...
            let ns = ns.clone();
            move |_, _, cx| {
              services::delete_job(name.clone(), ns.clone(), cx);
            }
          }))
      });
...
impl JobList {
  pub fn new(window: &mut Window, cx: &mut Context<'_, Self>) -> Self {
    ...
    cx.subscribe(&list_state, |_this, state, event: &ListEvent, cx| match event {
      ListEvent::Select(ix) | ListEvent::Confirm(ix) => {
        ...
        if let Some(j) = filtered.get(ix.row) {
          cx.emit(JobListEvent::Selected(j.clone()));
        }
      }
      ListEvent::Cancel => {}
    })
  ...
  fn ensure_search_input(&mut self, window: &mut Window, cx: &mut Context<'_, Self>) {
    if self.search_input.is_none() {
      let input_state = cx.new(|cx| InputState::new(window, cx).placeholder("Search jobs..."));
...
impl gpui::EventEmitter<JobListEvent> for JobList {}
    ...
    let toolbar = h_flex()
      ...
      .child(
        h_flex()
          ...
          .child(
            Button::new("jobs-toolbar-actions")
              .icon(IconName::Ellipsis)
              .ghost()
              .compact()
              .dropdown_menu(|menu, _, _| {
                menu.item(
                  PopupMenuItem::new("Refresh")
                    ...
                    .on_click(|_, _, cx| services::refresh_jobs(cx)),
                )
              }),
          ),
      );
