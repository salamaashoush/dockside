...
use crate::kubernetes::{JobInfo, PodInfo};
use crate::services;
use crate::state::{DockerState, JobDetailTab, StateChanged, docker_state};

pub struct JobDetail {
  docker_state: Entity<DockerState>,
  item: Option<JobInfo>,
  active_tab: JobDetailTab,
  yaml_content: String,
...
impl JobDetail {
  pub fn new(cx: &mut Context<'_, Self>) -> Self {
    ...
    cx.subscribe(&docker_state, |this, ds, event: &StateChanged, cx| match event {
      StateChanged::JobYamlLoaded { name, namespace, yaml } => {
        if let Some(ref j) = this.item
          && j.name == *name
      ...
      StateChanged::JobTabRequest { name, namespace, tab } => {
        let state = ds.read(cx);
        if let Some(j) = state.get_job(name, namespace) {
          this.item = Some(j.clone());
          this.active_tab = *tab;
      ...
      StateChanged::JobsUpdated => {
        if let Some(ref current) = this.item {
          let state = ds.read(cx);
          if let Some(updated) = state.get_job(&current.name, &current.namespace) {
  ...
  pub fn set_item(&mut self, item: JobInfo, cx: &mut Context<'_, Self>) {
    self.item = Some(item.clone());
    self.active_tab = JobDetailTab::Info;
    self.yaml_content.clear();
    self.yaml_editor = None;
    self.last_synced_yaml.clear();

    services::get_job_yaml(item.name, item.namespace, cx);
    cx.notify();
  }

  pub fn update_item(&mut self, item: JobInfo, cx: &mut Context<'_, Self>) {
    self.item = Some(item);
    cx.notify();
  }

  fn render_info_tab(item: &JobInfo, cx: &mut Context<'_, Self>) -> gpui::Div {
    ...
    let succeeded_color = if item.succeeded >= item.completions {
      colors.success
    } else {
      colors.warning
    };
    let failed_color = if item.failed > 0 {
      colors.danger
    } else {
    ...
    let mut content = v_flex()
      ...
      .child(info_row("Duration", item.duration.clone()))
              ...
              v_flex()
                ...
                .child(
                  div()
                    ...
                    .text_color(succeeded_color)
                    .child(item.succeeded.to_string()),
                )
                .child(div().text_xs().text_color(colors.muted_foreground).child("Succeeded")),
            )
            .child(
              v_flex()
                .items_center()
                ...
                .child(
                  div()
                    ...
                    .child(item.failed.to_string()),
                )
                .child(div().text_xs().text_color(colors.muted_foreground).child("Failed")),
            )
            .child(
              v_flex()
                .items_center()
                ...
                .child(
                  div()
                    ...
                    .font_weight(gpui::FontWeight::BOLD)
                    .text_color(colors.foreground)
                    .child(item.completions.to_string()),
                )
                .child(div().text_xs().text_color(colors.muted_foreground).child("Completions")),
            ),
        ),
    );
  ...
  fn render_pods_tab(&self, item: &JobInfo, cx: &mut Context<'_, Self>) -> gpui::Div {
    let colors = &cx.theme().colors;
    ...
    let matching: Vec<&PodInfo> = state
      ...
      .filter(|p| {
        ...
        // Jobs put job-name label on pods automatically
        p.labels.get("job-name").is_some_and(|v| v == &item.name)
          || p
            .labels
            .get("batch.kubernetes.io/job-name")
            .is_some_and(|v| v == &item.name)
      })
      .collect();
            ...
            div().w(px(40.)).flex().justify_end().child(
              Button::new(("view-pod", i))
                ...
                .on_click(move |_ev, _w, cx| {
                  services::open_pod_logs(pod_name.clone(), pod_namespace.clone(), cx);
                }),
            ),
          )
      })
      ...
      v_flex()
        ...
        .child(
          div()
            .text_xs()
            .text_color(colors.muted_foreground)
            .child(format!("{} pod(s) — click eye icon for logs", matching.len())),
        )
        .child(v_flex().w_full().child(header).children(rows)),
    )
  }

  fn render_yaml_tab(&self, item: &JobInfo, cx: &mut Context<'_, Self>) -> gpui::Div {
    let colors = &cx.theme().colors;
                  ...
                  PopupMenuItem::new("Apply YAML")
                    ...
                    .on_click(move |_, _, cx| {
                      ...
                      if !yaml.trim().is_empty() {
                        services::apply_job_yaml(apply_name.clone(), apply_namespace.clone(), yaml, cx);
                      }
                    }),
                )
                .separator()
                .item(PopupMenuItem::new("Reload from Cluster").on_click(move |_, _, cx| {
                  services::get_job_yaml(reload_name.clone(), reload_namespace.clone(), cx);
                }))
            }
          }),
      );
      ...
      v_flex()
        ...
        .child(
          div()
            .text_sm()
            .text_color(colors.muted_foreground)
            .child("Click on a job to view details"),
        ),
    )
  }
}

impl Render for JobDetail {
  fn render(&mut self, window: &mut Window, cx: &mut Context<'_, Self>) -> impl IntoElement {
    ...
    let tab_bar = TabBar::new("job-tabs")
      .flex_1()
      .py(px(0.))
      .child(
        Tab::new()
          .label("Info")
          .selected(active_tab == JobDetailTab::Info)
          .on_click(cx.listener(|this, _ev, _w, cx| {
            this.active_tab = JobDetailTab::Info;
            cx.notify();
          })),
      )
      .child(
        Tab::new()
          .label("Pods")
          .selected(active_tab == JobDetailTab::Pods)
          .on_click(cx.listener(|this, _ev, _w, cx| {
            this.active_tab = JobDetailTab::Pods;
            cx.notify();
          })),
      )
      .child(
        Tab::new()
          .label("YAML")
          .selected(active_tab == JobDetailTab::Yaml)
          .on_click(cx.listener(|this, _ev, _w, cx| {
            this.active_tab = JobDetailTab::Yaml;
            if let Some(ref item) = this.item {
              services::get_job_yaml(item.name.clone(), item.namespace.clone(), cx);
            }
          })),
      );

    let content = match active_tab {
      JobDetailTab::Info => Self::render_info_tab(&item, cx),
      JobDetailTab::Pods => self.render_pods_tab(&item, cx),
      JobDetailTab::Yaml => self.render_yaml_tab(&item, cx),
    };
