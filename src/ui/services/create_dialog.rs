    ...
    fn ensure_inputs(&mut self, window: &mut Window, cx: &mut Context<Self>) {
        if self.name_input.is_none() {
            self.name_input = Some(cx.new(|cx| {
                InputState::new(window, cx).placeholder("e.g. my-service")
            }));
        }
        ...
        if self.service_type_select.is_none() {
            self.service_type_select = Some(cx.new(|cx| {
                SelectState::new(ServiceType::all(), Some(IndexPath::new(0)), window, cx)
            }));
        }
        ...
        if self.target_port_input.is_none() {
            self.target_port_input = Some(cx.new(|cx| {
                InputState::new(window, cx).placeholder("8080")
            }));
        }
        if self.node_port_input.is_none() {
            self.node_port_input = Some(cx.new(|cx| {
                InputState::new(window, cx).placeholder("30000")
            }));
        }
        if self.selector_key_input.is_none() {
            self.selector_key_input = Some(cx.new(|cx| {
                InputState::new(window, cx).placeholder("app")
            }));
        }
        if self.selector_value_input.is_none() {
            self.selector_value_input = Some(cx.new(|cx| {
                InputState::new(window, cx).placeholder("my-app")
            }));
        }
    }
    ...
    pub fn get_options(&self, cx: &App) -> CreateServiceOptions {
        ...
        let ports: Vec<ServicePortConfig> = self
            ...
            .map(|p| ServicePortConfig {
                ...
                target_port: p.target_port.parse().unwrap_or_else(|_| {
                    p.port.parse().unwrap_or(80)
                }),
    ...
    fn render_general_tab(&self, colors: &DialogColors, _cx: &mut Context<Self>) -> impl IntoElement {
        ...
        v_flex()
            ...
            .child(self.render_form_row_with_desc(
                ...
                div()
                    .w(px(150.))
                    .child(Select::new(&service_type_select).small()),
                colors,
            ))
    }
                            ...
                            .child(
                                v_flex()
                                    ...
                                    .child(
                                        div()
                                            .w(px(60.))
                                            .child(Input::new(&target_port_input).small()),
                                    ),
                            )
                                ...
                                el.child(
                                    v_flex()
                                        ...
                                        .child(
                                            div()
                                                .w(px(60.))
                                                .child(Input::new(&node_port_input).small()),
                                        ),
                                )
                            })
                            .child(
                                ...
                                Button::new("add-port")
                                    ...
                                    .on_click(cx.listener(|this, _ev, window, cx| {
                                        ...
                                        if !port.is_empty() {
                                            ...
                                            this.port_input = Some(cx.new(|cx| {
                                                InputState::new(window, cx).placeholder("80")
                                            }));
                                            this.target_port_input = Some(cx.new(|cx| {
                                                InputState::new(window, cx).placeholder("8080")
                                            }));
                                            this.node_port_input = Some(cx.new(|cx| {
                                                InputState::new(window, cx).placeholder("30000")
                                            }));
                                            cx.notify();
                                        }
            ...
            .child(
                h_flex()
                    ...
                    .child(
                        div()
                            .w(px(120.))
                            .child(Input::new(&selector_key_input).small()),
                    )
                    .child(Label::new("=").text_color(muted_color))
                    .child(
                        div()
                            .flex_1()
                            .child(Input::new(&selector_value_input).small()),
                    )
                    .child(
                        Button::new("add-selector")
                            ...
                            .on_click(cx.listener(|this, _ev, window, cx| {
                                ...
                                if !key.is_empty() {
                                    ...
                                    this.selector_key_input = Some(cx.new(|cx| {
                                        InputState::new(window, cx).placeholder("app")
                                    }));
                                    this.selector_value_input = Some(cx.new(|cx| {
                                        InputState::new(window, cx).placeholder("my-app")
                                    }));
                                    cx.notify();
                                }
        ...
        v_flex()
            ...
            .child(
                div()
                    ...
                    .border_color(colors.border)
                    .child(
                        TabBar::new("create-service-tabs").children(
            ...
            .child(
                div()
                    ...
                    .when(active_tab == 2, |el| {
                        el.child(self.render_selectors_tab(&colors, cx))
                    }),
            )
    }
}
