...
impl ServicesView {
    pub fn new(window: &mut Window, cx: &mut Context<Self>) -> Self {
        ...
        let detail = cx.new(|cx| ServiceDetail::new(cx));

        // Subscribe to list events
        cx.subscribe_in(&list, window, |this, _list, event: &ServiceListEvent, window, cx| {
            match event {
                ...
                StateChanged::ServicesUpdated => {
                    ...
                    if let Some(ref current) = this.selected_service {
                        ...
                        this.selected_service =
                            state.get_service(&current.name, &current.namespace).cloned();
                        cx.notify();
                    }
                }
                _ => {}
    ...
    fn show_create_dialog(&mut self, window: &mut Window, cx: &mut Context<Self>) {
        let dialog_entity = cx.new(|cx| CreateServiceDialog::new(cx));
