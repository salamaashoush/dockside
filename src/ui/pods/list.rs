...
impl PodListDelegate {
    fn pods<'a>(&self, cx: &'a App) -> Vec<PodInfo> {
        ...
        let menu_button = Button::new(("menu", row))
            ...
            .dropdown_menu(move |menu, _window, _cx| {
                ...
                menu = menu
                    ...
                    .item(
                        PopupMenuItem::new("Force Delete")
                            .icon(IconName::Close)
                            .on_click({
                                let name = name.clone();
                                let ns = ns.clone();
...
impl PodList {
    pub fn new(window: &mut Window, cx: &mut Context<Self>) -> Self {
        ...
        cx.subscribe(&docker_state, |this, _state, event: &StateChanged, cx| {
            if matches!(
                event,
                StateChanged::PodsUpdated | StateChanged::NamespacesUpdated
            ) {
    ...
    fn ensure_search_input(&mut self, window: &mut Window, cx: &mut Context<Self>) {
        if self.search_input.is_none() {
            let input_state =
                cx.new(|cx| InputState::new(window, cx).placeholder("Search pods..."));
