...
pub enum SecretListEvent {
  Selected(SecretInfo),
}

pub struct SecretListDelegate {
  docker_state: Entity<DockerState>,
  search_query: String,
}

impl SecretListDelegate {
  fn items(&self, cx: &App) -> Vec<SecretInfo> {
  ...
  fn filtered(&self, cx: &App) -> Vec<SecretInfo> {
...
impl ListDelegate for SecretListDelegate {
  ...
  fn render_item(
    ...
    let is_selected =
      matches!(global_selection, Selection::Secret { name, namespace } if *name == s.name && *namespace == s.namespace);
    let name_clone = s.name.clone();
    let ns_clone = s.namespace.clone();
    let pin_favorite = crate::state::FavoriteRef::Secret {
      name: s.name.clone(),
    ...
    let subtitle = format!("{} • {} • {} keys", s.namespace, s.secret_type, key_count);
      ...
      .dropdown_menu(move |menu, _window, _cx| {
        ...
        menu
          .item(PopupMenuItem::new("View YAML").icon(IconName::File).on_click({
            ...
            let ns = ns.clone();
            move |_, _, cx| {
              services::open_secret_yaml(name.clone(), ns.clone(), cx);
            }
          }))
          .separator()
          .item(
          ...
          .item(PopupMenuItem::new("Delete").icon(Icon::new(AppIcon::Trash)).on_click({
            ...
            let ns = ns.clone();
            move |_, _, cx| {
              services::delete_secret(name.clone(), ns.clone(), cx);
            }
          }))
      });
...
pub struct SecretList {
  docker_state: Entity<DockerState>,
  list_state: Entity<ListState<SecretListDelegate>>,
...
impl SecretList {
  pub fn new(window: &mut Window, cx: &mut Context<'_, Self>) -> Self {
    ...
    let delegate = SecretListDelegate {
    ...
    cx.subscribe(&list_state, |_this, state, event: &ListEvent, cx| match event {
      ListEvent::Select(ix) | ListEvent::Confirm(ix) => {
        ...
        if let Some(s) = filtered.get(ix.row) {
          cx.emit(SecretListEvent::Selected(s.clone()));
        }
      }
      ListEvent::Cancel => {}
    })
  ...
  fn open_create_form(&mut self, window: &mut Window, cx: &mut Context<'_, Self>) {
    ...
    self.create_form = Some(KvFormState::new(&ns, false, window, cx));
    cx.notify();
  }
  ...
  fn submit_create_form(&mut self, cx: &mut Context<'_, Self>) {
    ...
    services::create_secret(name, namespace, entries, cx);
    self.create_form = None;
    cx.notify();
  }
  ...
  fn ensure_search_input(&mut self, window: &mut Window, cx: &mut Context<'_, Self>) {
    if self.search_input.is_none() {
      let input_state = cx.new(|cx| InputState::new(window, cx).placeholder("Search secrets..."));
...
impl gpui::EventEmitter<SecretListEvent> for SecretList {}

impl Render for SecretList {
  fn render(&mut self, window: &mut Window, cx: &mut Context<'_, Self>) -> impl IntoElement {
    let state = self.docker_state.read(cx);
    let total_count = state.secrets.len();
    let load_state = state.secrets_state.clone();
    ...
    let toolbar = h_flex()
      ...
      .child(
        h_flex()
          ...
          .child(
            Button::new("secret-toolbar-actions")
              .icon(IconName::Ellipsis)
              .ghost()
              .compact()
              .dropdown_menu(|menu, _, _| {
                menu.item(
                  PopupMenuItem::new("Refresh")
                    ...
                    .on_click(|_, _, cx| services::refresh_secrets(cx)),
                )
              }),
          ),
      );
