      ...
      LoadState::Loaded => {
        if secrets.is_empty() {
          ...
          let header = h_flex()
            ...
            .child(div().w(px(28.)));
          let mut rows = v_flex().w_full().min_w(px(800.));
          for s in &secrets {
            rows = rows.child(self.render_row(s, cx));
          }
          div().size_full().child(
            div().id("secrets-h-scroll").size_full().overflow_x_scroll().child(
              v_flex().w_full().min_w(px(800.)).size_full().child(header).child(
                div()
                  .id("secrets-v-scroll")
                  .flex_1()
                  .min_h_0()
