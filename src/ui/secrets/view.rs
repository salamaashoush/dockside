//! K8s Secret view: list + detail split.
...
use super::detail::SecretDetail;
use super::list::{SecretList, SecretListEvent};
...
pub struct SecretsView {
  ...
  list: Entity<SecretList>,
  detail: Entity<SecretDetail>,
}

impl SecretsView {
  fn selected(&self, cx: &App) -> Option<SecretInfo> {
    let state = self.docker_state.read(cx);
    if let Selection::Secret {
      ref name,
      ref namespace,
    } = state.selection
    {
      state.get_secret(name, namespace).cloned()
    } else {
      None
    }
  }
  ...
  pub fn new(window: &mut Window, cx: &mut Context<'_, Self>) -> Self {
    ...
    let list = cx.new(|cx| SecretList::new(window, cx));
    let detail = cx.new(SecretDetail::new);

    cx.subscribe_in(
      &list,
      window,
      |this, _list, event: &SecretListEvent, _window, cx| match event {
        SecretListEvent::Selected(item) => {
          let already_selected = matches!(
            this.docker_state.read(cx).selection,
            Selection::Secret { ref name, ref namespace } if *name == item.name && *namespace == item.namespace
          );
          if already_selected {
            return;
          }
          ...
          this.docker_state.update(cx, |state, _cx| {
            state.set_selection(Selection::Secret {
    ...
    cx.subscribe_in(
      ...
      |this, ds, event: &StateChanged, _w, cx| match event {
        StateChanged::SecretTabRequest {
          name,
          namespace,
          tab: _,
        } => {
          let opt = ds.read(cx).get_secret(name, namespace).cloned();
        ...
        StateChanged::SecretsUpdated => {
          let key = if let Selection::Secret {
            ref name,
            ref namespace,
          } = this.docker_state.read(cx).selection
          {
