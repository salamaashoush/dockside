...
impl ScaleDialog {
        ...
        if self.replicas_input.is_none() {
            self.replicas_input = Some(cx.new(|cx| {
                InputState::new(window, cx)
                    .default_value(&self.current_replicas.to_string())
            }));
        }
    }
    ...
    pub fn get_replicas(&self, cx: &App) -> i32 {
        self.replicas_input
            ...
            .unwrap_or_else(|| self.current_replicas.to_string())
            .parse::<i32>()
                    ...
                    .child(
                        h_flex()
                                    ...
                                    .on_click(cx.listener(|this, _ev, window, cx| {
                                        if let Some(input) = &this.replicas_input {
                                            ...
                                            if current > 0 {
                                                ...
                                                this.replicas_input = Some(cx.new(|cx| {
                                                    InputState::new(window, cx)
                                                        .default_value(&new_val.to_string())
                                                }));
                                                cx.notify();
                                            }
                                        }
                            ...
                            .child(
                                div()
                                    .w(px(80.))
                                    .child(Input::new(&replicas_input).small()),
                            )
                            .child(
                                Button::new("increase")
                                    .icon(IconName::Plus)
