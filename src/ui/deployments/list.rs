...
impl DeploymentList {
    ...
    fn ensure_search_input(&mut self, window: &mut Window, cx: &mut Context<Self>) {
        if self.search_input.is_none() {
            let input_state =
                cx.new(|cx| InputState::new(window, cx).placeholder("Search deployments..."));
    ...
    fn render_empty(&self, cx: &mut Context<Self>) -> gpui::Div {
        ...
        let (title, subtitle) = if !state.k8s_available {
            ...
            (
                "No Deployments",
                "Deploy an application to see deployments here",
            )
        };
