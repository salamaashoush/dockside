use gpui::{
    div, prelude::*, px, App, Context, Entity, FocusHandle, Focusable, Hsla, ParentElement,
    ...
    fn ensure_inputs(&mut self, window: &mut Window, cx: &mut Context<Self>) {
        if self.name_input.is_none() {
            self.name_input = Some(cx.new(|cx| {
                InputState::new(window, cx).placeholder("e.g. my-deployment")
            }));
        }
        ...
        if self.replicas_input.is_none() {
            self.replicas_input = Some(cx.new(|cx| {
                InputState::new(window, cx)
                    .placeholder("1")
                    .default_value("1")
            }));
        ...
        if self.pull_policy_select.is_none() {
            self.pull_policy_select = Some(cx.new(|cx| {
                SelectState::new(ImagePullPolicy::all(), Some(IndexPath::new(0)), window, cx)
            }));
        }
        if self.cpu_limit_input.is_none() {
            self.cpu_limit_input = Some(cx.new(|cx| {
                InputState::new(window, cx).placeholder("e.g. 500m")
            }));
        }
        if self.memory_limit_input.is_none() {
            self.memory_limit_input = Some(cx.new(|cx| {
                InputState::new(window, cx).placeholder("e.g. 256Mi")
            }));
        }
        if self.cpu_request_input.is_none() {
            self.cpu_request_input = Some(cx.new(|cx| {
                InputState::new(window, cx).placeholder("e.g. 100m")
            }));
        }
        if self.memory_request_input.is_none() {
            self.memory_request_input = Some(cx.new(|cx| {
                InputState::new(window, cx).placeholder("e.g. 128Mi")
            }));
        }
        if self.command_input.is_none() {
            self.command_input = Some(cx.new(|cx| {
                InputState::new(window, cx).placeholder("e.g. /bin/sh,-c")
            }));
        }
        if self.args_input.is_none() {
            self.args_input = Some(cx.new(|cx| {
                InputState::new(window, cx).placeholder("e.g. echo hello")
            }));
        }
        if self.port_name_input.is_none() {
            self.port_name_input = Some(cx.new(|cx| {
                InputState::new(window, cx).placeholder("http")
            }));
        }
        if self.port_number_input.is_none() {
            self.port_number_input = Some(cx.new(|cx| {
                InputState::new(window, cx).placeholder("80")
            }));
        }
        ...
        if self.label_key_input.is_none() {
            self.label_key_input = Some(cx.new(|cx| {
                InputState::new(window, cx).placeholder("key")
            }));
        }
        if self.label_value_input.is_none() {
            self.label_value_input = Some(cx.new(|cx| {
                InputState::new(window, cx).placeholder("value")
            }));
        }
    }
    ...
    fn render_general_tab(&self, colors: &DialogColors, _cx: &mut Context<Self>) -> impl IntoElement {
        ...
        v_flex()
            ...
            .child(self.render_form_row_with_desc(
                ...
                div()
                    .w(px(150.))
                    .child(Select::new(&pull_policy_select).small()),
                colors,
            ))
    }
            ...
            .child(
                h_flex()
                    ...
                    .child(
                        div()
                            .w(px(80.))
                            .child(Input::new(&port_name_input).small()),
                    )
                    .child(
                        div()
                            .w(px(80.))
                            .child(Input::new(&port_number_input).small()),
                    )
                    .child(
                        h_flex()
                            .gap(px(4.))
                        ...
                        Button::new("add-port")
                            ...
                            .on_click(cx.listener(|this, _ev, window, cx| {
                                ...
                                if !port.is_empty() {
                                    ...
                                    this.port_name_input = Some(cx.new(|cx| {
                                        InputState::new(window, cx).placeholder("http")
                                    }));
                                    this.port_number_input = Some(cx.new(|cx| {
                                        InputState::new(window, cx).placeholder("80")
                                    }));
                                    cx.notify();
                                }
            ...
            .child(
                h_flex()
                    ...
                    .child(
                        div()
                            .w(px(120.))
                            .child(Input::new(&label_key_input).small()),
                    )
                    .child(Label::new("=").text_color(muted_color))
                        ...
                        Button::new("add-label")
                            ...
                            .on_click(cx.listener(|this, _ev, window, cx| {
                                ...
                                if !key.is_empty() {
                                    ...
                                    this.label_key_input = Some(cx.new(|cx| {
                                        InputState::new(window, cx).placeholder("key")
                                    }));
                                    this.label_value_input = Some(cx.new(|cx| {
                                        InputState::new(window, cx).placeholder("value")
                                    }));
                                    cx.notify();
                                }
        ...
        v_flex()
            ...
            .child(
                div()
                    ...
                    .border_color(colors.border)
                    .child(
                        TabBar::new("create-deployment-tabs").children(
            ...
            .child(
                div()
                    ...
                    .when(active_tab == 0, |el| {
                        el.child(self.render_general_tab(&colors, cx))
                    })
                    .when(active_tab == 1, |el| {
                        el.child(self.render_resources_tab(&colors, cx))
                    })
                    .when(active_tab == 2, |el| {
                        el.child(self.render_ports_tab(&colors, cx))
                    })
                    .when(active_tab == 3, |el| {
                        el.child(self.render_env_tab(&colors, cx))
                    })
                    .when(active_tab == 4, |el| {
                        el.child(self.render_labels_tab(&colors, cx))
                    }),
            )
    }
}
